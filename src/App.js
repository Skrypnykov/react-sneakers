    ...
    async function fetchData() {
      try {
        const [cartResponse, favoritesResponse, itemsResponse] = await Promise.all([
          ...
          axios.get('http://localhost:3003/favorites'),
  ...
  const onAddToCart = async (obj) => {
    try {
      ...
      if (findItem) {
        ...
        await axios.delete(`http://localhost:3003/cart/${findItem.id}`);
      } else {
        setCartItems((prev) => [...prev, obj]);
        const { data } = await axios.post('http://localhost:3003/cart', obj);
        setCartItems((prev) =>
          prev.map((item) => {
  ...
  const onAddToFavorite = async (obj) => {
    try {
      if (favorites.find((favObj) => Number(favObj.id) === Number(obj.id))) {
        axios.delete(`http://localhost:3003/favorites/${obj.id}`);
